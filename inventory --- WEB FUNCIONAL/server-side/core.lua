...
Objects = {
	-- ROBBERY CLOTHESHOP
	["1"] = { x = 70.27, y = -1389.11, z = 29.13, h = 90.28, object = "p_v_43_safe_s", item = "", Distance = 50 }, -- 70.27,-1389.11,29.13
...
StealItens = {
	...
	{ ["item"] = "WEAPON_PISTOL_AMMO", ["min"] = 10, ["max"] = 30, ["rand"] = 150 },
...
-- REQUESTINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.requestInventory()
	local source = source
					...
					if Split[1] == "identity" or Split[1] == "fidentity" or string.sub(v["item"],1,5) == "badge" then
						...
						if Identity then
							...
							if Number == Passport and Split[1] == "identity" then
								...
								local hasPremiumGroup = vRP.HasGroup(Passport, "Premium")
							
								if hasPremiumGroup then
									v["idPremium"] = "Ativo"
								else
									v["idPremium"] = "Inativo"
								end
...
-- DROPSERVER
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.DropServer(Coords,Item,Amount)
	local Number = 0
...
-- DROPS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Drops(Item,Slot,Amount,x,y,z)
	local source = source
	...
	if Passport then
		if not Active[Passport] and not Player(source)["state"]["Handcuff"] and not exports["hud"]:Wanted(Passport) and not vRP.InsideVehicle(source) and GetPlayerRoutingBucket(source) < 900000 then
			...
			if itemBlocked(Item) then
				TriggerClientEvent('Notify', source, 'vermelho', 'Proibido Manipular item Vip', 5000)
				TriggerClientEvent("inventory:Update",source,"Backpack")
...
-- PICKUP
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Pickup(Number,Amount,Slot)
	local source = source
	local Slot = tostring(Slot)
...
-- SENDITEM
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.SendItem(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	...
	if Passport and not Active[Passport] and GetPlayerRoutingBucket(source) < 900000 then
		...
		if ClosestPed then
			...
			if itemBlocked(Item) then
				TriggerClientEvent('Notify', source, 'vermelho', 'Proibido manipular este item', 5000)
				TriggerClientEvent("inventory:Update",source,"Backpack")
...
-- DELIVER
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Deliver(Slot)
	local source = source
	local Slot = tostring(Slot)
	local Passport = vRP.Passport(source)
	if Passport then
...
-- TRASH
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Trash(Slot, Amount)
    local source = source
    local Slot = tostring(Slot)
    local Amount = parseInt(Amount)
    local Passport = vRP.Passport(source)
    ...
    if Passport then
        -- Removed the 'Wanted' check from 'hud'
...
-- USEITEM
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.UseItem(Slot,Amount)
	local source = source
	local Slot = tostring(Slot)
	local Amount = parseInt(Amount)
	local Passport = vRP.Passport(source)
...
-- CANCEL
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Cancel()
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-- CHECKINVENTORY
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.checkInventory()
	local source = source
	local Passport = vRP.Passport(source)
...
-- VERIFYWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.verifyWeapon(Item,Ammo)
	local source = source
	local Passport = vRP.Passport(source)
...
-- DROPWEAPONS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.dropWeapons(Item)
	local source = source
	local Passport = vRP.Passport(source)
...
-- REMOVETHROWING
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.removeThrowing(Item)
	local source = source
	local Passport = vRP.Passport(source)
...
-- PREVENTWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.preventWeapon(Item,Ammo)
	local source = source
	local Passport = vRP.Passport(source)
...
-- VERIFYOBJECTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.VerifyObjects(Entity,Service)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
...
-- STEALTRUNK
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.StealTrunk(Entity)
    local source = source
    local Plate = Entity[1]
    ...
    if not Active[Passport] then
        ...
        if os.time() >= Trunks[Plate] then
            ...
            if vTASKBAR.stealTrunk(source) then
                ...
                repeat
                    if os.time() >= tonumber(Active[Passport]) then
                        Active[Passport] = nil
                        vRPC.stopAnim(source, false)
                        Player(source)["state"]["Buttons"] = false
...
-- ANIMALS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Animals(Entity)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-- STOREOBJECTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.StoreObjects(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-- MAKEPRODUCTS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.MakeProducts(Table)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and not Active[Passport] then
...
-- DISMANTLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Dismantle(Entity)
	local source = source
...
-- REMOVETYRES
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.RemoveTyres(Entity)
	local source = source
	local Passport = vRP.Passport(source)
...
-- STEALPEDS
-----------------------------------------------------------------------------------------------------------------------------------------
-- function Creativenetwork.StealPeds()
-- 	local source = source
-- 	local Passport = vRP.Passport(source)
-- 	if Passport then
...
-- CHECKDRUGS
-----------------------------------------------------------------------------------------------------------------------------------------
 function Creativenetwork.CheckDrugs()
 	local source = source
 	local Passport = vRP.Passport(source)
 	if Passport then
...
-- -- PAYMENTDRUGS
-- -----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.PaymentDrugs()
    local source = source
    local Passport = vRP.Passport(source)
    ...
    if Passport and Drugs[Passport] then
        ...
        if vRP.TakeItem(Passport, Drugs[Passport][1], Drugs[Passport][2], true) then
            ...
            for Passport, Source in pairs(Service) do
                local teupai = vRP.Passport(Source)
                if vRP.HasService(teupai, "Police") then
...
-- REGISTERSTIMERS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.RegistersTimers(Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
-- REGISTERSPAY
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.RegistersPay()
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
...
exports('dropPlayerWeapons', function(source, Passport)
	local inventory = vRP.Inventory(Passport)
	 for slot, infos in pairs(inventory) do
		 local Item = infos.item
