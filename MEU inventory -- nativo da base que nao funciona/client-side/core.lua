...
-----------------------------------------------------------------------------------------------------------------------------------------
Creativenetwork = {}
Tunnel.bindInterface("inventory",Creativenetwork)
...
RegisterCommand("openBackpack", function()
	if not IsPauseMenuActive() and GetEntityHealth(PlayerPedId()) > 100 and not LocalPlayer["state"]["Buttons"] and not LocalPlayer["state"]["Commands"] and not LocalPlayer["state"]["Handcuff"] and not IsPlayerFreeAiming(PlayerId()) then
		...
		-- Adicionando o som ao abrir a mochila
		TriggerEvent("sounds:source", "chest", 0.35)  -- Dispara o som relacionado ao "chest"
	end
end)
...
-- PARACHUTE
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.parachuteColors()
	local Ped = PlayerPedId()
...
-- FISHINGCOORDS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Fishing()
	local Ped = PlayerPedId()
...
-- FISHINGANIM
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.fishingAnim()
	local Ped = PlayerPedId()
...
-- ANIMALANIM
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.animalAnim()
	local Ped = PlayerPedId()
...
-- RETURNWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.returnWeapon()
	if Weapon ~= "" then
		return Weapon
	end
...
-- CHECKWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.checkWeapon(Hash)
	if Weapon == Hash then
		return true
	end
...
AddEventHandler('Weapon:Attachs', function() 
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            Citizen.Wait(optimization) -- Aguarde o tempo definido antes de repetir o loop
        end
    end)
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- PUTWEAPONHANDS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.putWeaponHands(Name,Ammo,Components,Type)
	if not TakeWeapon then
		...
		if HasPedGotWeapon(Ped,GetHashKey("GADGET_PARACHUTE"),false) then
			RemoveAllPedWeapons(Ped,true)
			Creativenetwork.parachuteColors()
		else
			RemoveAllPedWeapons(Ped,true)
		end
		...
		if Components then
			for nameItem,_ in pairs(Components) do
				Creativenetwork.putAttachs(nameItem,Name)
			end
		end
...
-- STOREWEAPONHANDS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.storeWeaponHands()
...
-- RECHARGECHECK
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.rechargeCheck(ammoType)
	local weaponAmmo = 0
...
-- RECHARGEWEAPON
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.rechargeWeapon(Hash,Ammo)
...
-- CHECKCRACKER
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.checkCracker()
...
-- CHECKWATER
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.checkWater()
		...
		if LocalPlayer["state"]["Route"] < 900000 then
			...
			for _,v in pairs(Drops) do
				...
				if Distance <= 50 then
					...
					DrawMarker(21, v["Coords"][1], v["Coords"][2], v["Coords"][3] + 0.25, 0.0, 0.0, 0.0, 0.0, 180.0, 0.0, 0.20, 0.20, 0.20, 255, 255, 255, 125, 0, 0, 0, 1)
				end
			end
		end
...
-- WHEELCHAIR
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.wheelChair(Plate)
	local Ped = PlayerPedId()
...
-- TYRESTATUS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.tyreStatus()
	local Ped = PlayerPedId()
	if not IsPedInAnyVehicle(Ped) then
...
-- TYREHEALTH
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.tyreHealth(Network,Tyre)
	if NetworkDoesNetworkIdExist(Network) then
		local Vehicle = NetToEnt(Network)
		if DoesEntityExist(Vehicle) then
...
-- DOESOBJECTEXIST
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.objectExist(Coords,Hash)
...
-- CHECKATM
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.checkAtm(Coords)
...
-- DISMANTLE
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Dismantle(Experience)
...
-- DISMANTLESTATUS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.DismantleStatus()
	return disActive
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- INVENTORY:DISRESET
...
local WeaConfig = {
	...
	["WEAPON_COLTXM177"] = { -- COLT SEM COLETE
		["Bone"] = 24818,
		["x"] = 0.12,
		["y"] = -0.14,
		["z"] = 0.04,
	...
	["WEAPON_SPECIALCARBINE"] = { -- AR 15 SEM COLETE
		["Bone"] = 24818,
		["x"] = 0.0,
		["y"] = -0.14,
		["z"] = -0.17,
	...
	["WEAPON_ARPLATINA"] = { -- WEAPON_ARPLATINA
		["Bone"] = 24818,
		["x"] = 0.0,
		["y"] = -0.14,
		["z"] = -0.17,
	...
	["WEAPON_FALAMERICA"] = { -- WEAPON_FALAMERICA
		["Bone"] = 24818,
		["x"] = 0.0,
		["y"] = -0.14,
		["z"] = -0.17,
	...
	["WEAPON_FALCAMMO"] = { -- WEAPON_FALCAMMO
		["Bone"] = 24818,
		["x"] = 0.0,
		["y"] = -0.14,
		["z"] = -0.17,
	...
	["WEAPON_PARAFALF"] = { --  WEAPON_PARAFALF
		["Bone"] = 24818, 
		["x"] = 0.12,
		["y"] = -0.14,
		["z"] = 0.04,
	...
	["WEAPON_M16VERDE"] = { -- WEAPON_M16VERDE
		["Bone"] = 24818,
		["x"] = 0.0,
		["y"] = -0.14,
		["z"] = -0.17,
	...
	["WEAPON_SPECIALCARBINE_MK2"] = { -- PARAFAL DE LUNETA E CAMUFLADO
		["Bone"] = 24818, -- SEM COLETE
		["x"] = 0.12,
		["y"] = -0.14,
		["z"] = 0.04,
		["RotX"] = 180.0,
...
local WeaConfig2 = {
	...
	["WEAPON_COLTXM177"] = { -- COLT COM COLETE
		["Bone2"] = 24818,
		["x"] = 0.0,
		["y"] = -0.14,
		["z"] = -0.17,
	...
	["WEAPON_SPECIALCARBINE"] = { -- AR15 COM COLETE
		["Bone2"] = 24818,
		["x"] = 0.0,
		["y"] = -0.14,
		["z"] = -0.17,
	...
	["WEAPON_G3A"] = { -- OMBRO COM BICO PARA BAIXO
		["Bone2"] = 24818, -- COM COLETE
		["x"] = 0.1,
		["y"] = 0.20,
		["z"] = 0.0,
		["RotX"] = 160.0,
	...
	["WEAPON_SPECIALCARBINE_MK2"] = { -- PARAFAL CAMUFLADO COM LUNETA e COLETE
		["Bone2"] = 24818,
		["x"] = 0.12,
		["y"] = -0.17,
		["z"] = 0.04,
	...
	["WEAPON_PARAFAL"] = { -- OMBRO COM BICO PARA BAIXO
		["Bone2"] = 24818, -- COM COLETE
		["x"] = 0.1,
		["y"] = 0.20,
		["z"] = 0.0,
...
AddEventHandler("inventory:RemoveWeapon",function(Item)
	...
	if WeaObjects2[Name] then
		TriggerServerEvent("DeleteObject",0,Name)
		WeaObjects2[Name] = nil
	end
end)
...
AddEventHandler("inventory:CreateWeapon",function(Item)
	...
	if not WeaObjects[Name] and WeaConfig[Name] then
		...
		local Config2 = WeaConfig2[Name]
		local Coords = GetEntityCoords(Ped)
		local Bone = GetPedBoneIndex(Ped,Config["Bone"])
		local Bone2 = GetPedBoneIndex(Ped,Config2["Bone2"])

		local Progression,Network = vRPS.CreateObject(Config["Model"],Coords["x"],Coords["y"],Coords["z"],Name)
		if Progression then
		--	if GetPedDrawableVariation(Ped,9) == coletes  then
		local draw = GetPedDrawableVariation(Ped, 9)
			if draw >= 7 and draw<= 85 then
				WeaObjects2[Name] = LoadNetwork(Network)
				AttachEntityToEntity(WeaObjects2[Name],Ped,Bone2,Config2["x"],Config2["y"],Config2["z"],Config2["RotX"],Config2["RotY"],Config2["RotZ"],false,false,false,false,2,true)
				SetEntityCompletelyDisableCollision(WeaObjects2[Name],false,true)
			else
...
-- CHECKMODS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.CheckMods(Vehicle,Mod)
	return GetNumVehicleMods(Vehicle,Mod) - 1
end
-----------------------------------------------------------------------------------------------------------------------------------------
-- CHECKCAR
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.CheckCar(Vehicle)
	local Model = GetEntityModel(Vehicle)
...
-- ACTIVEMODS
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.ActiveMods(Vehicle,Plate,Mod,Number)
	if NetworkDoesNetworkIdExist(Vehicle) then
		local Vehicle = NetToEnt(Vehicle)
		if DoesEntityExist(Vehicle) then
			if GetVehicleNumberPlateText(Vehicle) == Plate then
...
-- RESTAURANT
-----------------------------------------------------------------------------------------------------------------------------------------
function Creativenetwork.Restaurant(Name)
	local Return = false
	local Ped = PlayerPedId()
	local Coords = GetEntityCoords(Ped)
	
	if Name == "Catcafe" then
